...
Características:
 - Acepta 0..N archivos por cada fuente; funciona si pasas 1, 2 o las 3 fuentes
...
Ejemplo de uso en línea de comandos:
  python unify_sources.py --sprinklr XandNewsCO.xlsx --tubular tubular1.xlsx --youscan youscan.csv -o unified.xlsx

Ejemplo de uso programático en Python/Colab:
  from unify_sources import unify_files
  result = unify_files(
      sprinklr_files=["archivo1.xlsx", "archivo2.xlsx"],
...
def unify_and_export(
    ...
    if sprinklr_files:
        df_s = _process_for_source(
            ...
            created_col=created_col_spr, skiprows=skiprows, header=header,
    ...
    if tubular_files:
        df_t = _process_for_source(
            ...
            created_col=created_col_tub, skiprows=skiprows, header=header,
            tz_from=tz_from, tz_to=tz_to, drop_original_created=drop_original_created
        )
        if df_t is not None:
            sheets["tubular"] = df_t
    ...
    if youscan_files:
        ...
        df_y = _process_for_source(
            ...
            created_col=None, skiprows=skiprows, header=header,
            tz_from=tz_from, tz_to=tz_to, drop_original_created=drop_original_created,
            extra_args=extra
        )
        if df_y is not None:
...
def unify_files(
    ...
    Args:
        ...
        skiprows: Número de filas a saltar al leer archivos
...
def main(argv: Optional[List[str]] = None):
    parser = argparse.ArgumentParser(
        description="Unifica Sprinklr/Tubular/YouScan en un único .xlsx"
    )
